...
http {
    ...
    server {
        ...
        location /media {
            ...
            # Originals and download files are only reachable through the
            # API's download route (counted + named), never directly
            location ^~ /media/original/ { return 404; }
            location ^~ /media/download/ { return 404; }
        }

        # Download files: reached only via X-Accel-Redirect from
        # GET /api/media/{id}/download. Logged to bandwidth.log so whole-file
        # downloads are metered like HLS segments.
